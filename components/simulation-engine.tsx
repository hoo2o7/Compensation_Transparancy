...
export function SimulationEngine() {
  ...
  const { transparency, readability, contributions } = useMemo(() => {
    const contributions = DRIVERS.map((d) => {
      ...
      const shap = d.weight * ((clampedValue - d.baseline) / (d.max - d.min))
      return { ...d, value, shap }
    }).sort((a, b) => Math.abs(b.shap) - Math.abs(a.shap))

    const total = contributions.reduce((sum, c) => sum + c.shap, 0)
    ...
    return { transparency: finalTransparency, readability: finalReadability, contributions }
  }, [metrics])
